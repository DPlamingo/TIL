...
def create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # 이때, article이 가지고 있어야 할 user_id를 안넣고 생성할려고 하니
            
            # 그럼 article에 user 정보 (요청한 유저)를 담아야 하니까
            # 일단 article 생성은 하는데... db에는 보내지말자//
            article = form.save(commit=False)
            # 생성된 article 객체의 user 속성에 요청보낸 request.user 정보를 담자.
            # 담은 것은 python의 article 변수의 값에만 할당 한것.
            # db에 반영
...
def detail(request, article_pk):
    # SELECT * FROM articles_article WHERE id = article_pk;
...
def likes(request, article_pk):
    ...
    # 이미 request.user와 article이 M:N 관계를 맺고 있는 상태일 때,
